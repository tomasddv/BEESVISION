...
const buildAnomalyIndex = (rows) => {
  ...
  const anomalies = rows.map((row) => {
    const image = getField(
      row,
      ["photo_image_url", "link imagen", "imagen", "image url", "image", "foto", "url", "photo"],
      ""
    );
    const type = classifyAnomalyType(
      getField(row, ["tipo anomaly", "tipo anomalia", "anomaly type", "motivo", "reason", "anomaly_detected"], "")
    );
    const dateValue = getField(row, ["task_executed_datetime", "fecha", "fecha ejecucion", "date"], "");
    const item = {
      image,
      type,
      promoter: normalizeText(getField(row, ["promotor"], "")),
      task: normalizeText(getField(row, ["task_name", "tarea", "detalle tarea", "task"], "")),
...
const getImageValue = (row) =>
  getField(row, ["photo_image_url", "link imagen", "imagen", "image url", "image", "foto", "url", "photo"], "");
...
export const processDatasets = ({ mainRows = [], clientRows = [], reviewRows = [], anomalyRows = [] }) => {
  ...
  const tasks = mainRows
    ...
    .filter((row) => ["Disponibilidad", "Precio", "Frio"].includes(row.pillar));
  ...
  const reviews = reviewRows
    .map((row) => {
      ...
      if (commentKey) {
        reviewed = "Si";
        if (commentKey.includes("falla") && commentKey.includes("algoritmo")) {
